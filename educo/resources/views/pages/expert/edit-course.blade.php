    ...
    <div>
        ...
        @if ($step === '1')
            ...
            <form action="/expert/course/update/{{ $course_id }}" method="POST" enctype="multipart/form-data"
                ...
                @php
                    function hasPropertyValue($objects, $property, $value): bool
                    {
                ...
                <select class="rounded-md w-1/5" name="skills" id="skills" name="skills" multiple>
                    @foreach ($skills as $key => $skill)
                        @php
                            $hasSkill = false;
                            foreach ($courseHasSkills as $key => $courseSkill) {
                                if ($courseSkill->skill_id === $skill->id) {
                                    $hasSkill = true;
                                }
                            }
                        @endphp
                        <option {{ $hasSkill === true ? 'selected' : null }} value="{{ $skill->id }}">
                            {{ $skill->title }}
                        </option>
                    @endforeach
                </select>
